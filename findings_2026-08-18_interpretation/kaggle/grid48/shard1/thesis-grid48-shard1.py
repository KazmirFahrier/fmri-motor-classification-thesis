...
The first sweep found `32^3` beats the disclosed `24^3` by `+0.0135`, selected in 29 of
30 folds, and noted that `32^3` was the largest grid tested so the optimum might lie
beyond it. This extends the sweep to `48^3` — `110592` features — to find out whether the
gain continues or turns over.

Only one grid is extracted here because `48^3` is large: roughly 170 MB per subject, and
the full event x lag x feature array would be about 10 GB. The analysis kernel reduces to
the lag-mean one subject at a time for exactly that reason.
