"""Native-resolution smoothing on the finer grid: the two largest preprocessing wins together.
...
It did beat it, and by a wide margin: at 24^3, native sigma 1.4 reaches 0.8648 against
0.8275 for the post-hoc box filter and 0.8098 unsmoothed. Combining the two made things
*worse* (0.8475), so they are substitutes rather than complements — smoothing twice
over-smooths.

Separately, the grid sweep found 32^3 beats the disclosed 24^3. Those are the two largest
preprocessing effects left in the project and they act on different axes — resolution and
smoothing extent — so whether they combine is an open question rather than an obvious
one. This extraction puts them together.
...
# Sigma in voxels of the acquired volume. The 24^3 run found accuracy still rising at
# sigma 1.4 (0.8098 -> 0.8545 -> 0.8638 -> 0.8648), so the range is extended upward here
# to find where it turns over. Sigma 0 is retained as the validation anchor: at 32^3 it
# must reproduce the known 0.8233.
SIGMAS = [0.0, 1.4, 1.8, 2.2]
