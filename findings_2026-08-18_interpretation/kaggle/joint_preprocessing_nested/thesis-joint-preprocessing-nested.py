...
# ====================================================================================
"""Jointly select spatial resolution and native smoothing inside subject folds.

Separate confirmatory runs established that native Gaussian smoothing survives nested
selection at 24 cubed and that the finest tested unsmoothed grid survives nested
selection across 16, 24, 32, and 48 cubed. This experiment closes the remaining
interaction by selecting from both native smoothing extractions in the same inner
subject folds. No held out subject contributes to the candidate choice.

The primary metric is independent balanced accuracy. Balanced assignment is retained
as a secondary complete run protocol. Per subject rows are saved for paired bootstrap
inference and for the final public benchmark.
"""
import json
import numpy as np
from collections import defaultdict
...
SOURCE_FAMILIES = [
    ...
    },
    {
        ...
        "path_filter": "grid48",
        "expected_sigma0": 0.8447,
    },
]
TOLERANCE = 0.004


def candidate_name(grid, sigma):
    return f"grid{grid}_sigma{sigma}"


blocks, candidates, anchors = {}, [], {}
y, records = None, None
for family in SOURCE_FAMILIES:
    grid = family["grid"]
    anchors[candidate_name(grid, 0.0)] = family["expected_sigma0"]
    for sigma, key in family["entries"]:
        ...
        if y is None:
            ...
            assert np.array_equal(labels, y), f"{name}: labels differ"
            assert recs == records, f"{name}: records differ"
        blocks[name] = block
...
for name in candidates:
    fixed[name] = evaluate(blocks[name], y, records, splits, class_count, label=name)
for name, expected in anchors.items():
    observed = float(np.mean(fixed[name]["independent"]))
    if abs(observed - expected) > TOLERANCE:
        raise SystemExit(
            f"VALIDATION FAILED: {name} {observed:.4f} != {expected:.4f}"
        )
    print(f"VALIDATION PASSED: {name} {observed:.4f}", flush=True)
...
for split in splits:
    ...
    print(f"{split['split']} selected {best} -> "
          f"{nested['independent'][-1]:.4f}", flush=True)

summary = {
    name: {rule: float(np.mean(values)) for rule, values in fixed[name].items()}
    for name in candidates
}
summary["nested"] = {r: float(np.mean(v)) for r, v in nested.items()}
counts = {name: selected.count(name) for name in candidates}
json.dump({
    "protocol": {
        "outer_folds": 30,
...
}, open("/kaggle/working/joint_preprocessing_nested.json", "w"), indent=2)

print("\n  candidate             independent   balanced")
for name in candidates:
    print(f"  {name:<21} {summary[name]['independent']:.4f}        "
          f"{summary[name]['balanced']:.4f}")
print(f"  nested    {summary['nested']['independent']:.4f}        "
      f"{summary['nested']['balanced']:.4f}")
print(f"selected: {counts}")
best_fixed = max(candidates, key=lambda name: summary[name]["independent"])
print(f"\noracle candidate {best_fixed}: {summary[best_fixed]['independent']:.4f}")
print(f"nested:            {summary['nested']['independent']:.4f}")
print(f"nesting cost:      "
      f"{summary[best_fixed]['independent'] - summary['nested']['independent']:+.4f}")
