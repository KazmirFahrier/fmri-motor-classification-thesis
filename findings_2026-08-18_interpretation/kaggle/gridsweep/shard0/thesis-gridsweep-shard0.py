...
Three grids are extracted in a single pass per volume, so the download cost is paid once
rather than three times. `24^3` is included deliberately: it must reproduce the frozen
checkpoints exactly, and if it does not, nothing else in the sweep can be trusted.
...
GRIDS = [(16, 16, 16), (24, 24, 24), (32, 32, 32)]
