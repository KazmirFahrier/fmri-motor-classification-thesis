...
- Treat labeled subject calibration as a serious modeling track. The current best is light source/subject centroid blending (`alpha=0.25`) plus per-run balanced assignment; the next step is to make this protocol more realistic by validating calibration-run count, choosing `alpha` without test labels, and testing whether unlabeled or semi-supervised calibration can approach the labeled result.
