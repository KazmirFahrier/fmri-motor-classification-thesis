...
- Treat labeled subject calibration as a serious modeling track. The current best is light source/subject centroid blending (`alpha=0.25`) plus per-run balanced assignment, and the validation-selected variant remains above `0.70` with at least three calibration runs. The next step is to validate calibration-run count and test whether unlabeled or semi-supervised calibration can approach the labeled result.
