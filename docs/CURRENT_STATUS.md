...
| Corrected clip feature transfer | Dense full-cohort overlapping clips show strong within-run signal. Dense `24³` offset-2 per-subject-run centering plus cosine reaches `0.6022` subject-fold independent accuracy and `0.6370` with known-design balancing. Adding unlabeled linear event-time detrending raises these to `0.6661` and `0.7176`, with `0.7655` held-out-run balanced accuracy; shuffled-time controls fail. Adding labeled subject calibration reaches `0.7984` with five target-subject runs (`0.7957` with validation-selected alpha). The main remaining problem is fine-grained within-pair separation plus QC for irreparable runs. |
