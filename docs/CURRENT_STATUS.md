...
| Corrected clip feature transfer | Dense full-cohort overlapping clips show strong within-run signal. Dense `24³` per-subject-run centering plus cosine reaches `0.5669` subject-fold event accuracy with event-mean features, but using only late clip `offset 2` reaches `0.6022`. Offset-2 plus known-design balancing reaches `0.6376` subject-fold event accuracy and `0.6851` held-out-run event accuracy. With labeled subject calibration, offset-2 source/subject centroid blending plus balanced assignment reaches `0.7224` when five target-subject calibration runs are available. Coarse leg-vs-arm accuracy is now above `0.92` in the calibrated protocol, so the remaining problem is mostly fine-grained within-pair separation plus QC for internally inconsistent subjects. |
