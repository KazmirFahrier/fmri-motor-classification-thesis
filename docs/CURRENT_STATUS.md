...
Last updated: 2026-05-07 23:05 EDT.
...
| Full-dataset subject-wise evaluation | [`kazmirfahrierniloy/thesis-7batch-subjectwise-gpucompat-resume`](https://www.kaggle.com/code/kazmirfahrierniloy/thesis-7batch-subjectwise-gpucompat-resume) | Running after CUDA determinism fixes | Continue leakage-aware subject-wise 5-fold evaluation plus holdout. |
...
- The subject-wise relauncher was patched locally to set `CUBLAS_WORKSPACE_CONFIG=:4096:8`, then the subject-wise config was changed to `training.deterministic: false` because strict deterministic CUDA max-pool backward is unsupported. Version 3 is now running.
...
| Full-dataset subject-wise CV | Fold 0 snapshot: accuracy 0.25, balanced accuracy 0.25, macro F1 0.10. Fold 1 is running after CUDA determinism fixes; final 5-fold plus holdout bundle is required before drawing conclusions. |
