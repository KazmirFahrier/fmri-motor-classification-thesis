...
- Detrending and labeled personalization are complementary. On detrended features, source-only leave-one-subject balanced accuracy is `0.7204`; fixed five-run calibration reaches `0.7984`, and validation-selected calibration reaches `0.7957`. `sub-42` and `sub-52` remain near chance, so the paper should separate personalization-responsive subjects from residual QC failures.
