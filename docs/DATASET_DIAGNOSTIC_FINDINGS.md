...
A remote header audit then examined all raw, preprocessed-T1w, and denoised-T1w files for `sub-30`, `sub-42`, `sub-52`, and `sub-62` without downloading image payloads. Within every subject/stage, all six runs have identical shapes, voxel sizes, qform/sform matrices, and axis codes; maximum affine difference is zero. No duplicate S3 ETags indicate accidental file reuse. One derivative anomaly is real but not explanatory: `sub-42/run-05` preprocessed BOLD is stored as `float32` and is about `341 MB`, while the other preprocessed runs are `int16` and about `177 MB`. Run 5 is one of the stronger detrended `sub-42` runs, and all denoised files are consistently `float32`.

All six denoised runs for `sub-42`, `sub-52`, and stable `sub-62` were then streamed one at a time, reduced to compact mean/std/mask maps, and deleted. Gross spatial consistency is high in every subject: mean mask Dice is `0.970`, `0.969`, and `0.969`, while mean temporal-mean-map correlation is `0.994`, `0.996`, and `0.995`. This rules out a large orientation, affine, brain-coverage, or registration failure.

The failures do show subtler instability. Mean temporal-variance-map correlation is `0.796` for `sub-42`, `0.839` for `sub-52`, and `0.879` for `sub-62`. Mean mask center-of-mass displacement is `0.299 mm`, `0.624 mm`, and `0.113 mm`; `sub-52` reaches `1.547 mm` between runs 2 and 6. Across all 45 run pairs, variance-map correlation associates with identity-class correspondence (`r=0.374`, rank `0.498`) and center shift associates negatively (`r=-0.315`, rank `-0.454`). These relationships are not stable within each subject, so they are forensic clues rather than proof of causation.

Finally, matched fMRIPrep-preprocessed and custom-denoised event patterns were compared for six targeted runs using a common mask, the validated `3:8` window, and true-time linear detrending. Denoising improves same-minus-different geometry in every run and never lowers leave-one-event accuracy. Examples include `sub-52/run-02` (`0.557` to `0.714`, accuracy `0.750` to `0.875`) and stable `sub-62/run-03` (`0.600` to `0.636`, accuracy unchanged at `1.0`). Catastrophic `sub-42/run-02` is already weak after fMRIPrep (`0.131` / `0.375`) and modestly improves after denoising (`0.152` / `0.500`). Therefore the custom denoising derivative does not create the failure; it is present earlier in the source/preprocessed signal.

The public derivative contains no confound TSVs or transform sidecars, preventing a formal framewise-displacement or transform-parameter audit. Further motion analysis must derive proxies directly from image volumes or recover the original fMRIPrep outputs. At this point the evidence favors run-varying acquisition/task-response instability over a deterministic extraction, timing, affine, registration, or denoising bug.

Raw-image motion forensics then covered all six runs of `sub-42`, `sub-52`, stable `sub-30`, and stable `sub-62`. Two independent translation proxies were used alongside normalized DVARS: coarse FFT phase correlation and center-of-signal trajectories. Mean phase framewise-displacement p95 is `0.089`, `0.067`, `0.054`, and `0.086 mm`, respectively; mean center-trajectory framewise p95 is `0.349`, `0.260`, `0.265`, and `0.387 mm`. The quietest subject is catastrophic `sub-52`, while stable `sub-62` has the largest center-framewise value and highest DVARS. Gross source motion is therefore not a discriminator.

Motion-only event classification also fails to account for the neural patterns. Same-subject leave-one-run accuracy from ten event-level DVARS/translation summaries is `0.479` for `sub-30`, `0.333` for `sub-42`, `0.188` for `sub-52`, and `0.354` for `sub-62`. Motion-template similarity appears related to neural-template similarity when all 60 run pairs are pooled (`r=0.651`), but this is an ecological/between-subject effect: the corresponding within-subject correlations are only `0.269`, `0.249`, `0.226`, and `-0.094`, and motion identity matching does not track neural identity matching consistently.

A train-only nuisance-regression experiment sharpened the interpretation. Fitting motion-to-voxel coefficients on five runs and applying them to the unseen sixth run at ridge `10` reduces stable classification from `0.875` to `0.604` for `sub-30` and from `0.833` to `0.354` for `sub-62`; both drops are below all 50 within-run shuffled-motion controls (`p=0.0196`). However, the motion covariates contain class means because executing different motor tasks can change both BOLD activation and body/head response. After removing every class mean from motion within each run, leaving only repetition-level excess motion, the same regression leaves stable accuracy at `0.917` and `0.833`. It neither explains nor repairs `sub-42`/`sub-52`. Thus unconditioned motion regression subtracts class-linked task signal and is not evidence that the classifier is driven by artifact; repetition-level motion nuisance is not the catastrophic mechanism.

The raw audit was then extended to the prespecified weak runs outside the catastrophic pair: `sub-54` runs `1/4/6`, `sub-63` runs `1/4`, and `sub-20/run-03`, with stable `sub-62/run-03` unchanged as a reference. Every run is weak before detrending, but they split after repair. The best tested detrended windows raise `sub-54/run-01` and `run-04` to same-minus-different geometry `0.484/0.529` with `0.625` leave-one-event accuracy, and `sub-63/run-01` to `0.566/0.500`. By contrast, `sub-54/run-06`, `sub-63/run-04`, and `sub-20/run-03` remain at only `0.001/-0.034/-0.073` geometry and `0.125/0.500/0.125` accuracy. The residual failures are not a shared HRF-window error.

Image-derived motion was also expanded to all six runs of `sub-20`, `sub-54`, and `sub-63`. Their mean phase-translation p95 values are modest (`0.056`, `0.044`, and `0.074 mm`). More importantly, motion and repaired event geometry do not have a consistent relationship: pooled within-subject-centered correlations are `-0.197` for phase-FD p95, `-0.350` for center-FD p95, and `-0.249` for normalized DVARS p95, while subject-specific signs differ. The quiet `sub-20/run-03` is the weakest targeted raw run; `sub-54`'s quiet late runs are worse than its more displaced early runs. `sub-63` may have some motion-linked degradation, but that subject-specific clue cannot justify a cohort-wide threshold or regression. The defensible post-repair QC policy is therefore to keep all runs in the primary estimate, retain unlabeled linear detrending, and flag residual weak runs for response-topography/run-state analysis rather than automatically excluding them.

Finally, run-to-run correspondence was recomputed for every saved continuous-BOLD window. Across offsets `2-6` and lengths `2-8`, `sub-42` remains at `0.167-0.271` leave-one-run accuracy and `sub-52` at `0.146-0.250`. Their identity-optimal run-pair fractions never exceed `0.133` and `0.0`, respectively. Stable `sub-30` remains `0.875-0.958` and stable `sub-62` `0.625-0.833`. No available latency/window choice restores the failed subjects, so the remaining source-level investigation should focus on response amplitude and anatomy rather than another temporal-offset sweep.

Full-cohort response-topography decomposition then separated within-run signal quality from cross-run direction. `sub-42` and `sub-52` have ordinary within-run repeat cosine (`0.430` and `0.414`), class-centroid RMS (`0.0149` and `0.0116`), spatial concentration, and edge energy. These values are comparable to stable `sub-30`/`sub-62`. Their own-other-run class-map alignment is instead negative (`-0.071` and `-0.049`), compared with `0.473` and `0.440` in the controls. The failed runs therefore contain strong, repeatable patterns, but the class-specific directions change between runs.

The authors' experiment source code resolves the remaining label-provenance concern. Task IDs are fixed in this order: toe, ankle, left leg, right leg, forearm, upper arm, wrist, finger, eye, jaw, lip, and tongue. Condition order is randomized within each run, but task IDs and stimulus textures are not remapped. The project's numeric event decoder is correct.

Most importantly, an independent audit streamed the dataset authors' official level-2 HCP/Ciftify z-stat maps for all 62 subjects. After centering the four maps across class within each subject, `sub-52` and `sub-42` are the worst and second-worst subjects in the cohort for leave-subject-out topographic identity similarity (`-0.093` and `-0.027`); the third-worst subject is already positive at `0.265`. Stable `sub-30` and `sub-62` score `0.642` and `0.640`. Class-centered map RMS is also bottom-two: `sub-42` is at the 1.6th percentile and `sub-52` at the 3.2nd percentile, even though their mean positive z-stat values are not extreme. Thus the anomaly is reproduced by the authors' canonical six-run GLM, independently of this project's extraction, detrending, temporal window, resizing, or classifier.
...
A nested spatial-scale investigation then tested native `24^3` features, `3^3`/`5^3` local smoothing, and `2^3`/`3^3`/`4^3` block pooling. Applying one transform globally does not help: repeated `2^3` pooling gives `0.7615` deployable fused accuracy versus `0.7626` at native scale, and stronger smoothing/pooling loses fine structure. However, `2^3` pooling improves the exact-class oracle-coarse branch, showing that within-leg/within-arm discrimination benefits from local aggregation while the leg-versus-arm gate needs native resolution.

The final multi-scale hierarchy therefore uses native features for the coarse gate and `3^3` smoothed features for the two pair specialists. Coarse and pair feature counts, plus fusion weights, are selected entirely inside inner training-subject folds. An explicit hierarchical balanced decoder uses the known experiment composition: each complete run contains four leg and four arm target events, with two events for each exact class. It first balances the native coarse assignment, then balances each smoothed pair assignment; no held-out labels are used.

Adding nested coarse feature selection raises repeated hard-balanced accuracy to `0.8201` and fixed-partition accuracy to `0.8269`. Relative to the previous native model (`0.7626` repeated), the paired gain is `0.0575`, its bootstrap interval is `0.0472-0.0673`, and the hybrid wins 29 of 30 folds with one tie. Every seed improves. Subject-averaged accuracy is `0.8207` across all 62 subjects (95% subject-bootstrap interval `0.7839-0.8536`) and `0.8390` in the 60-subject QC stratum (`0.8142-0.8640`). Independent decoding without the known balance constraint is `0.7557` and must be reported for settings where run composition is unknown.
...
Arm-specific temporal screening does not reveal a better response window either. The canonical `offset 2, length 6` window lowers fixed fused accuracy from `0.8759` to `0.8482`, with leg/arm pair accuracy falling from `0.8648`/`0.8045` to `0.8283`/`0.7903`. Holding offset 3 while shortening the integration to six volumes gives `0.8780` fused accuracy but lowers pair accuracy to `0.8547`/`0.8004` and independent accuracy to `0.8164`. The small balanced change is assignment noise rather than improved arm separation. Keep `offset 3, length 8` for both branches.

A reproducible subject-residual analyzer now aggregates each subject's five held-out confusion matrices and separates coarse, leg-pair, and arm-pair performance. Under the validated 1,024-feature hierarchy, mean subject-level exact accuracy is `0.8757`, leg-pair accuracy `0.9237`, and arm-pair accuracy `0.8277`. Nineteen subjects meet the prespecified arm-specific pattern (leg at least `0.75` and arm at least `0.15` lower), two are leg-specific, 39 are mixed/mild, and only `sub-42`/`sub-52` remain below `0.50` globally. The worst non-global arm cases are `sub-39` (`0.500` arm), `sub-44` (`0.617`), `sub-02`/`sub-34` (`0.633`), and `sub-57` (`0.667`).

The independent official-GLM audit explains the two global failures but not the residual arm subgroup. Across all 62 subjects, official group-identity similarity correlates with exact accuracy at `r=0.628`, but after excluding only the independently verified `sub-42`/`sub-52` outliers this falls to `r=0.168` (rank `0.114`). Its QC-stratum association with arm accuracy is only `r=0.146` (rank `0.105`); class-centered GLM amplitude is similarly weak (`r=0.020`, rank `0.239`). Thus arm-specific errors among otherwise usable subjects are not a simple continuation of the catastrophic topography-quality axis. The next experiment should test nested arm calibration or arm-focused learned representations, not broader QC exclusion.

A labeled target-subject arm-calibration positive control now preserves the source-subject feature ranking and regularized covariance while blending only forearm and upper-arm class means estimated from non-evaluation runs. Every target evaluation run is excluded from both calibration and alpha selection. Source-only balanced arm accuracy is `0.8414`. Validation-selected blending reaches `0.8464` with two calibration runs, `0.8621` with three, `0.8661` with four, and `0.8804` with all five available runs; independent arm accuracy reaches `0.8320` with five runs.

At the subject level, five-run validated calibration improves balanced arm accuracy by `0.0390` across all 62 subjects (95% subject-bootstrap interval `0.0108-0.0699`) and by `0.0333` in the 60-subject QC stratum (`0.0069-0.0625`). The 19 post-hoc arm-specific subjects improve by `0.0833` (`0.0132-0.1535`), from `0.7237` source-only to `0.8070`; this subgroup result is mechanistically useful but not a confirmatory generalization estimate because the subgroup was defined from prior errors. Three-run calibration gains `0.0207` overall with an interval crossing zero slightly (`-0.0030-0.0460`), while its arm-specific gain is `0.0570` (`0.0009-0.1145`).
...
The arm-calibration hypothesis now survives an exact four-class end-to-end test. The evaluator reconstructs the saved repeated hierarchy baseline exactly at `0.875164` under fold weighting, then updates only target-subject pair means from runs other than the evaluation run. Calibration-only leave-one-run-out alpha selection raises subject-averaged balanced exact accuracy from `0.8757` to `0.8861` with three runs, `0.8892` with four, and `0.8913` with five. The five-run paired subject gain is `0.0157` with a 95% interval of `0.0049-0.0269`; independent exact accuracy rises from `0.8250` to `0.8430`, and every repeated seed improves.

The controls sharpen the interpretation. Calibrating the strong leg branch is neutral or harmful, while arm-only calibration is more stable than updating both pairs. In the QC-60 sensitivity stratum, four-run arm calibration reaches `0.9065` and five-run calibration `0.9088`, both with positive paired intervals. One run still harms the exact model and two runs remain inconclusive. Five-run calibration helps 31 subjects, ties six, and hurts 25, so the result is a statistically supported personalization ceiling rather than a universal adaptation policy. The next question is whether calibration-run quality can gate the light arm update without examining the held-out run.

That calibration-only gate question is now answered negatively for the simplest defensible diagnostic. Leave-one-calibration-run-out improvement has only `0.03-0.06` correlation with the exact-class gain on the unseen run. Applying calibration only when the internal gain is positive uses the update in 43.7% of five-run cases and lowers balanced exact accuracy to `0.8836`, versus `0.8913` for universal validation-selected arm calibration. A threshold selected from every other subject instead applies calibration in 99.9% of cases and reaches `0.8911`, which is functionally the universal policy. Internal run consistency does not identify the subjects that regress. With three to five labeled runs, retain the light arm update; future safety work needs a genuinely different predictor or hierarchical Bayesian shrinkage rather than another threshold on calibration accuracy.
...
Because the saved windows are exact voxelwise means, the two-volume late tail can be reconstructed algebraically as `(8 * mean[3:8] - 6 * mean[3:6]) / 2`. Nested candidates combine the main response amplitude with this tail or its contrast against the first six late volumes. Tail-based representations are selected in four of six fixed folds, showing apparent inner-fold structure, but outer balanced arm accuracy is `0.8342` versus `0.8354` baseline and independent accuracy falls from `0.8045` to `0.7961`. The tail effect is therefore another overfitting signal, not a transferable temporal representation.

The full ordered response is now available rather than inferred from overlapping means. Sequence-only extraction produced 62 checkpoints of shape `(48 events, 8 lags, 13,824 voxels)`, with 2,976 total events and no integrity errors. Averaging these sequences exactly reconstructs the saved repeated hierarchy baseline at `0.875164`, establishing that the new artifact has not changed preprocessing or event identity.

A source-subject-only temporal Fisher direction then learns one arm-specific weight per lag before the spatial full-covariance classifier. The regularized filter is highly stable across all 30 folds (minimum pairwise cosine `0.990`) and resembles a gently late-weighted mean, but reaches only `0.8740` balanced accuracy versus `0.8752` for the uniform mean. Independent accuracy changes from `0.8248` to `0.8238`, and arm-pair performance is slightly lower. Removing temporal smoothness and using an SVD signal direction also remain below baseline (`0.8736` and `0.8737`). Ordered timing is therefore not useless, but no single cohort-level temporal projection improves transfer. Any further sequence model must represent lag-specific spatial structure or multiple temporal components rather than optimize another global HRF-like vector.

The low-rank follow-up identifies one complementary component. The first Fisher direction remains mean-like; the second is a stable early-to-late contrast whose largest magnitude is at lag 7 in every outer fold. Using that contrast alone destroys arm discrimination (`0.8031` exact balanced), but pairing it with a uniform mean reaches `0.8795` balanced accuracy at the 1,024-feature cap. All five repeated seeds improve, and the paired fold interval is positive, although the subject-bootstrap interval still crosses zero. Adding third and fourth temporal components does not help.

The bounded 2,048-feature interaction raises independent exact decoding to `0.8316`, a `+0.00684` gain over the original 1,024-feature mean hierarchy with paired interval `+0.00258-0.01100`. Balanced accuracy reaches `0.8799`, but its comparison with the primary balanced model remains uncertain. Subject-averaged all-62/QC-60 results are `0.8802`/`0.8989` balanced and `0.8319`/`0.8491` independent. This is the strongest zero-label independent model and a credible temporal mechanism, but the conservative primary balanced estimate remains `0.8752` until rank/cap selection is nested or confirmed in another cohort.

Nested candidate selection now removes the rank/cap choice from held-out evaluation. Four inner subject folds choose among the original mean-cap-1024 model and mean-plus-contrast models at caps 1,024 and 2,048. Temporal candidates are selected in 26 of 30 outer folds, including cap 2,048 in 21. Selected accuracy is `0.8806` balanced and `0.8314` independent. Independent gain over the original primary model is positive under both fold (`0.00326-0.01039`) and subject (`0.00249-0.01082`) bootstrap, with 40 subjects improving. Balanced fold uncertainty is positive, but its subject interval still crosses zero. The temporal contrast is therefore a confirmed transferable feature for ordinary independent decoding, while its smaller design-balanced benefit remains supportive rather than a replacement primary claim.

Candidate selection should remain shared and balanced-oriented. Choosing a separate candidate from inner independent accuracy increases 2,048-cap temporal selection from 21 to 25 folds but lowers outer independent accuracy from `0.8314` to `0.8307`. Although that rule-specific control still beats the original mean model, it over-selects the larger covariance candidate. Balanced exact accuracy supplies the more stable inner regularizer for both deployment rules.

The weak-run forensic decomposition motivated one further design-aware decoder. Several residual runs have inconsistent individual repetitions even when their averaged class direction still aligns with the correct subject template: all four class-repeat cosines are negative in `sub-20/run-03`, three are negative in `sub-54/run-06`, and `sub-63` has broader instability in runs `1/4/6`. A new exact assignment solver therefore scores every one of the 2,520 legal two-events-per-class assignments by hierarchy evidence plus within-assigned-class event similarity. The consistency weight is selected using four inner subject folds; held-out labels do not enter either similarity or selection.

The zero-weight solver exactly reconstructs all 30 stored temporal-hierarchy folds, and nonzero weights are selected in every outer fold (`0.25` in 22, `0.5` in 8). Repeated balanced accuracy rises from `0.8806` to `0.8948`, with a paired fold interval of `+0.0093` to `+0.0191` and subject interval of `+0.0077` to `+0.0204`. All five seeds improve; 42 subjects improve, 6 tie, and 14 worsen. Subject-averaged all/QC-60 accuracy is `0.8950/0.9142`. Gains occur for every class, including arm recalls increasing from `0.8382/0.8363` to `0.8578/0.8551`. This is now the strongest zero-label complete-run decoder, but it assumes the known balanced block design and transductively compares target-run events. Independent accuracy remains `0.8314` for unbalanced, incomplete, or online settings.

A nonlinear diagonal-QDA control then tests whether forearm and upper-arm classes differ through class-specific voxel variance. Full LDA and QDA compete with nested feature-count and shrinkage selection. Full LDA is selected for both leg and arm in all six outer folds, and every aggregate metric exactly reproduces the established fixed baseline (`0.87595` fused balanced, `0.82216` independent). Class-dependent diagonal variance contributes no useful generalization. With concatenation, explicit tail contrasts, and QDA all negative, further transformations of event means have diminishing scientific value; a genuinely new temporal model requires per-volume sequences from the continuous BOLD source.
